...
y = np.array([6, 13, 18, 28, 52, 53, 61, 60.0])
n = np.array([59, 60, 62, 56, 63, 59, 62, 60.0])
...
def main_10(theta_init, n=10000, burn=1000, call_from_other=False):
    ...
    def compute_r10(theta, sigma):
        ...
        jump_theta = np.array([1.8, -4.0, -1.0])
        ...
        #r_num = np.array(r_num, dtype = np.float128)
        #r_denom = np.array(r_denom, dtype = np.float128)

        r_num = np.array(r_num, dtype = float)
        r_denom = np.array(r_denom, dtype = float)
