...
def MonteCarlo(P,Q,R,S,N,X0,Y0,Z0,Xbar,Zbar,Mu,Var,alpha,delta,reps,T, YICgen,full=2):
    """
    ...
    TODO:
        Make the moment calculations a running average instead
        of appending the new value to a list each time.

        Check to see other things and see if the above line is applicable.
    """
    full = int(full)
    T = int(T)
    T=full*T
    ...
    plt.figure(1)
    ax = plt.subplot(111)
    ax.plot(x,GDP_lower[start:-1], label = 'Lower Band') # ll =
    ax.plot(x,GDP_upper[start:-1], label = 'Upper Band') # mm =
    ax.plot(x,GDP_mean[start:], label = 'GDP') # aa =
    ...
    plt.figure(2)
    ax = plt.subplot(111)
    ax.plot(x,invest_lower[start:-1], label = 'Lower Band')
    ax.plot(x,invest_upper[start:-1], label = 'Upper Band')
    ax.plot(x,invest_mean[start:], label = 'Investment')
    ...
    plt.figure(3)
    ax = plt.subplot(111)
    ax.plot(x,consumption_lower[start:-1], label = 'Lower Band')
    ax.plot(x,consumption_upper[start:-1], label = 'Upper Band')
    ax.plot(x,consumption_mean[start:], label = 'Consumption')
